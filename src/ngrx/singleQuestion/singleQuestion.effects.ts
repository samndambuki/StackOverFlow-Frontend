    ...
    this.actions$.pipe(
      ...
      switchMap(({ questionId }) =>
        this.singleQuestionService
          .getAnswersByQuestionId(questionId, this.token)
          .pipe(
            map((answers) =>
  ...
  postAnswer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ questionId, answer }) =>
        this.singleQuestionService
          .postAnswer(questionId, answer, this.token)
          .pipe(
            map((response) =>
              SingleQuestionActions.postAnswerSuccess({ response })
            ),
            catchError((error) =>
              of(
                SingleQuestionActions.postAnswerFailure({
                  error: error.message,
                })
              )
            )
